...
objects.create = function (username, object, source, callback) {
	...
	DB.objects.insert({object : doc}, doc.path, function (err, body) {
		if (err) {
			logger.warn(err);
			return callback(err);
		}
...
objects.delete = function (username, object, callback) {
	'use strict';
	DB.destroyDoc(DB.objects, username + '/' + object, function (body, err) {
		if (err) {
			logger.warn(err);
			return callback(false, err);
		}
		DB.indexes.get(username, function (e, b) {
			...
			b.objects = b.objects.filter(function (obj) {
				if (obj.id !== body.object.id) {
					return obj;
				}
			});
			DB.indexes.insert({objects: b.objects, "_rev" : b._rev}, username, function (ee, bb) {
