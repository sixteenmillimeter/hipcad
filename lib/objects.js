...
objects.create = function (username, object, source, callback) {
	...
	indexesGetCb = function (e, b) {
		if (e) {
			logger.warn(e);
			return callback(e);
		}
		b.objects.push(objindex);
		DB.indexes.insert({objects: b.objects, "_rev" : b._rev}, username, indexesInsertCb);
	},
	indexesInsertCb = function (ee, bb) {
		if (ee) {
			logger.warn(ee);
			return callback(ee);
		}
		logger.info('Created object ' + username + '/' + object);
		callback(null, bb.objects);
	};
