...
controller.fail = function (res : Response, msg : any, status : number, json : any) {
	var page;
	if (json) {
...
controller.home = function (req : Request, res : Response, next : Function) {
	let pageData : any = {};
	let page : any;
	let tag : any;
	let recaptcha : any;
	...
	var tagUserCb = function (req : Request, res : Response, tagOutput : any) {
		tag = tagOutput;
		controller.auth(req, res, authCb);
	},
	authCb = function (err : Error, auth : boolean) {
		if (auth) {
...
controller.user = {};
controller.user.get = function (req : Request, res : Response, next : Function) {
	...
	tagUserCb = function (req : Request, res : Response, tagOutput : any) {
		tag = tagOutput;
	...
	userExistsCb = function (uexists : boolean) {
		if (uexists) {
	...
	userIndexCb = function (err : Error, data : any) {
		//TODO: handle err
		if (json) {
...
controller.object.get = async function (req, res, next) {
	...
	try{
		...
		logObj.err = err;
		log.error(`Error determining if object exists`, logObj);
...
controller.object.create = function (req, res) {
	...
	objectsCreateCb = function (err, data) {
		if (err) {
			logObj.status = 500;
			log.warn('controller.object.create', logObj);
			log.error(err);
			return controller.fail(res, 'Server error', 500, json);
		}
		logObj.tag = tag;
...
controller.login = function (req, res) {
	...
	usersLoginCb = function (err, success) {
		...
		if (success) {
			...
			logObj.status = 401.1;
			log.warn('users.login callback success false');
			log.warn('controller.login', logObj);
...
module.exports = async (pool) => {
	...
	hipcad.tmpl = require('./templates')(pool);
