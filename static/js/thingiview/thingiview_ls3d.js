 ...
 * By M. McWilliams
 ---------------------------------------------------*/
/*global scope, Thingiview, $, document, window*/


if (scope === undefined) { var scope; }
 ...
 *
 * Creates THREE camera, scene, lights and wrapping 
 * elements
 ---------------------------------------------------*/
 ...
 *
 * On resize event (as declared above) sets size of renderer to new height
 * Adjusts the perspective of the THREE camera
 * Runs scene loop
 *----------------------------------------------------------------------------*/
  // FIXME
  ...
  //   }    
  // };
 var onContainerResize = function(width, height) {
	//width  = parseFloat(document.defaultView.getComputedStyle(container,null).getPropertyValue('width'));
	//height = parseFloat(document.defaultView.getComputedStyle(container,null).getPropertyValue('height'));
 ...
 *
 * Prevents scroll, evaluates event delta and if positive or negative and zooms
 * +/-10 cameraZoom units
 *----------------------------------------------------------------------------*/
...
/*------------------------------------------------------------------------------
 * onRendererGestureChange()
 *
 * @param   event   Touch event
 * 
 * Prevents gesture and +/- 5 cameraZoom units based on whether or not event 
 * scale parameter is over 1
 *----------------------------------------------------------------------------*/
 ...
 *----------------------------------------------------------------------------*/
  var onRendererMouseOver = function(event) {
	...
	//if (timer == null) {
	  // log('starting loop');
	  //timer = setInterval(sceneLoop, 1000/frame);
	//}
  };
 ...
 *----------------------------------------------------------------------------*/
  this.setRotation = function(rotate) {
	var rotation = rotate;
	if (rotate) {
	  rotateTimer = setInterval(rotateLoop, 1000/frame);
	} else {
 ...
 *----------------------------------------------------------------------------*/
  ...
  this.setCameraPos = function (obj) {
	object.rotation.x = obj.objRot.x;
	plane.rotation.x = obj.planeRot.x;
	camera.position.x = obj.cameraPos.x;

	object.rotation.y = obj.objRot.y;
	plane.rotation.y = obj.planeRot.y;
	camera.position.y = obj.cameraPos.y;

	object.rotation.z = obj.objRot.z;
	plane.rotation.z = obj.planeRot.z;
	camera.position.z = obj.cameraPos.z;
//
	//console.dir(object);
	//console.dir(plane);
	//console.dir(camera);

	targetYRotation = obj.targetY;
	targetXRotation = obj.targetX;
  ...
  this.getCameraPos = function () {
  	return {
  		...
  		targetY : targetYRotation,
  		targetX : targetXRotation
  	};
  };
 ...
 *----------------------------------------------------------------------------*/
  this.loadSTL = function(url) {
	//Good place to track down saving state
  ...
  function loadObjectGeometry() {
	if (scene && geometry) {
	  if (objectMaterial == 'wireframe') {
		...
		var material = new THREE.MeshBasicMaterial({color:objectColor,wireframe:true});
	  } else {
		if (isWebGl) {
...
/*------------------------------------------------------------------------------
 * log
 * Sends message to console.log and fires viewermsg for ls3d app
 *
 *----------------------------------------------------------------------------*/
function log(msg) {
  if (this.console) {
  ...
  $(document).trigger('viewermsg', [msg]);
}

/*------------------------------------------------------------------------------
 * WorkerFacade
 *
 * Fakes WebWorkers when not supported by browsers
 *----------------------------------------------------------------------------*/
...
/*------------------------------------------------------------------------------
 * TODO: 
 * o Documentation pass
 * o Expose camera state
 *    o List used attributes
 * o Restore camera state
 * o Pass configure object ala codemirror
 * o Pause camera on move (configuration);
 * o Remove guess work
------------------------------------------------------------------------------*/
